...
```
    ...
    * dockerfile.push.skip
    ...
    # docker build -t 127.0.0.1:8850/sca/gateway:0.0.1-SNAPSHOT .	
```

2.mvn deploy 会执行 ``dockerfile:push`` 命令
```
# mvn deploy会直接push，但是有个bug暂时没解决
docker push 127.0.0.1:8850/sca/gateway:0.0.1-SNAPSHOT
```

3.运行镜像
```
docker run --name sca-gateway -itd 127.0.0.1:8850/sca/gateway:0.0.1-SNAPSHOT \
